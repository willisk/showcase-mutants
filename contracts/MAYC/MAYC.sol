...
contract Mutants is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    event MutantPublicSaleStart(uint256 indexed _saleDuration, uint256 indexed _saleStartTime);
    event MutantPublicSalePaused(uint256 indexed _currentPrice, uint256 indexed _timeElapsed);
    event StartingIndicesSet(uint256 indexed _mintedMutantsStartingIndex, uint256 indexed _megaMutantsStartingIndex);

    modifier whenPublicSaleActive() {
        require(publicSaleActive, 'Public PUBLIC_SALE_NOT_ACTIVE');
        _;
    }

    modifier startingIndicesNotSet() {
        require(mintedMutantsStartingIndex == 0, 'Minted Mutants starting index is already set');
        require(megaMutantsStartingIndex == 0, 'Mega Mutants starting index is already set');
        _;
    }
    ...
    function getMintPrice() public view whenPublicSaleActive returns (uint256) {
        ...
        if (elapsed >= publicSaleDuration) {
            ...
            uint256 currentPrice = ((publicSaleDuration - elapsed) * publicSaleMutantStartingPrice) / publicSaleDuration;
            return currentPrice > PS_MUTANT_ENDING_PRICE ? currentPrice : PS_MUTANT_ENDING_PRICE;
        }
    }
    ...
    function mintMutants(uint256 numMutants) external payable whenPublicSaleActive nonReentrant {
        require(numMutantsMinted + numMutants <= PS_MAX_MUTANTS, 'Minting would exceed max supply');
        require(numMutants > 0, 'Must mint at least one mutant');
        require(numMutants <= PS_MAX_MUTANT_PURCHASE, 'Requested number exceeds maximum');
    ...
    function mutateApeWithSerum(uint256 serumTypeId, uint256 apeId) external nonReentrant {
        require(serumMutationActive, 'Serum Mutation is not active');
        require(bayc.ownerOf(apeId) == msg.sender, "Must own the ape you're attempting to mutate");
        require(bacc.balanceOf(msg.sender, serumTypeId) > 0, 'Must own at least one of this serum type to mutate');

        uint256 mutantId;

        if (serumTypeId == MEGA_MUTATION_TYPE) {
            require(currentMegaMutationId <= MAX_MEGA_MUTATION_ID, 'Would exceed supply of serum-mutatable MEGA MUTANTS');
            require(megaMutationIdsByApe[apeId] == 0, 'Ape already mutated with MEGA MUTATION SERUM');
            ...
            require(!_exists(mutantId), 'Ape already mutated with this type of serum');
        }
    ...
    function getMutantIdForApeAndSerumCombination(uint256 apeId, uint8 serumTypeId) external view returns (uint256) {
        uint256 mutantId;
        if (serumTypeId == MEGA_MUTATION_TYPE) {
    ...
    function hasApeBeenMutatedWithType(uint8 serumType, uint256 apeId) external view returns (bool) {
    ...
    function getMutantId(uint256 serumType, uint256 apeId) internal pure returns (uint256) {
        require(serumType != MEGA_MUTATION_TYPE, "Mega mutant ID can't be calculated");
        return (apeId * NUM_MUTANT_TYPES) + serumType + SERUM_MUTATION_OFFSET;
    }

    function isMinted(uint256 tokenId) external view returns (bool) {
        require(tokenId < MAX_MEGA_MUTATION_ID, 'tokenId outside collection bounds');
    ...
    function calculateStartingIndex(uint256 blockNumber, uint256 collectionSize) internal view returns (uint256) {
    ...
    function setStartingIndices() external startingIndicesNotSet {
        ...
        uint256 elapsed = getElapsedSaleTime();
        require(elapsed >= publicSaleDuration && publicSaleStartTime > 0, 'Invalid setStartingIndices conditions');

        mintedMutantsStartingIndex = calculateStartingIndex(collectionStartingIndexBlock, PS_MAX_MUTANTS);

        megaMutantsStartingIndex = calculateStartingIndex(collectionStartingIndexBlock, NUM_MEGA_MUTANTS);

        if ((block.number - collectionStartingIndexBlock) > 255) {
            mintedMutantsStartingIndex = calculateStartingIndex(block.number - 1, PS_MAX_MUTANTS);

            megaMutantsStartingIndex = calculateStartingIndex(block.number - 1, NUM_MEGA_MUTANTS);
        }
        ...
        emit StartingIndicesSet(mintedMutantsStartingIndex, megaMutantsStartingIndex);
    }
}
