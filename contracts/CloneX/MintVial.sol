...
// https://etherscan.io/address/0x348fc118bcc65a92dc033a951af153d14d945312#code
...
contract Mintvial is ERC1155, Ownable, ERC1155Burnable {
    ...
    constructor()
        ERC1155("ipfs://QmQqMF7izNAaU9CY3qV9ZGs4Aksv6ywjx8261khgzQbReW")
    {
        // ERC-721
    ...
    // Authorize specific smart contract to be used for minting an ERC-1155 token
    function toggleContractAuthorization(
        address contractAddress,
        uint256 typeOfContract
    ) public onlyOwner {
    ...
    function mint(
        ...
        if (publicSales == true) {
            ...
            require(
                amount <= 3 && amount >= 1,
                "You have to mint between 1 to 3 at a time"
            );
        } else {
            // Else if the public sales is not active
            require(privateSales, "Public sales has not started");
            require(contractIds.length > 0, "No contract detected");
            require(
                tokenIds.length == contractIds.length,
                "tokenIds must match contractIds length"
            );
            ...
            for (uint256 i = 0; i < contractIds.length; i++) {
                ...
                if (authorizedContract[contractIds[i]] == 721) {
                    ...
                    require(
                        contractAddress.ownerOf(tokenIds[i]) == msg.sender,
                    ...
                    require(
                        contractAddress.balanceOf(msg.sender, tokenIds[i]) > 0,
                        "Doesn't own the token"
                    );
                } else {
                ...
                require(
                    checkIfRedeemed(contractIds[i], tokenIds[i]) == false,
                ...
                if (
                    contractIds[i] == 0x20fd8d8076538B0b365f2ddd77C8F4339f22B970
                ) amount += 1;
                else amount += 3;
            }
        }
    ...
    function airdropGiveaway(
        ...
        for (uint256 i = 0; i < to.length; i++) {
            tokenId++;
            require(
                amountToMint[i] + amountMinted <= limitAmount,
                "Limit reached"
            );
    ...
    function migrateToken(uint256 id) public returns (uint256) {
        ...
        ClonexCharacterInterface clonexContract = ClonexCharacterInterface(
            clonexContractAddress
        );
    ...
    function checkIfRedeemed(address _contractAddress, uint256 _tokenId)
        public
        view
        returns (bool)
    {
